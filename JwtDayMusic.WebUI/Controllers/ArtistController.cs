...
namespace JwtDayMusic.WebUI.Controllers
{
    public class ArtistController : Controller
    {
        ...
        public async Task<IActionResult> ArtistList()
        {
            ...
            ViewBag.FollowingIds = await GetFollowingIdsAsync();
            return View(artists);
        }
        ...
        private async Task<HashSet<int>> GetFollowingIdsAsync()
        {
            var token = _httpContextAccessor.HttpContext.Session.GetString("JwtToken");
            ...
            var response = await client.GetAsync("https://localhost:7180/api/Artist/following");
